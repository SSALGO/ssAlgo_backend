...
class NorenApi:
    ...
    '''def __ws_run_forever(self):
        while not self.__stop_event.is_set():
            ...
            try:
                ...
                #logger.exception("Exception during WebSocket run forever:")
                # self.close_websocket()
            ...
            try:
                if not self.__websocket_connected:
                    self.__websocket.run_forever(
                        ...
                        ping_payload='{"t":"h"}',
                        reconnect=5,
                        sslopt=self.__ws_sslopt,
                    )
                    if not self.__websocket_connected and not self.__stop_event.is_set():
                        self.websocketoffed = True
                        sleep(2)
                ...
                if 'socket is already opened' in str(e):
                    logger.warning(f"{e}")
                    # Handle the case when the socket is already opened
                else:
                    # Handle other exceptions and decide whether to attempt reconnection
                    logger.warning(f"websocket run forever ended in exception, {e}")

                # Additional reconnection logic here
    ...
    def restart_websocket(self):
        ...
        # Sleep for a short duration before starting a new WebSocket
        sleep(1)

        # Start a new WebSocket
        self.start_websocket(
            subscribe_callback=self.__subscribe_callback,
            order_update_callback=self.__order_update_callback,
            socket_open_callback=self.__on_open,
            socket_close_callback=self.__on_disconnect,
            socket_error_callback=self.__on_error
        )
        self.websocketoffed = False
