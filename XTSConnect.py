...
class XTSConnect(XTSCommon):
    ...
    def __init__(self, 
                 ...
                 root_uri="https://xts-api.example.com",  # Default root API
                 disable_ssl=False,
                 timeout=7):
        """
        Initialize the API wrapper with custom or default settings.
        """
        super().__init__(token, userID, isInvestorClient)

        self._root_uri = root_uri
        self._login_uri = self._root_uri + "/user/session"
        self._timeout = timeout
        self._ssl_flag = disable_ssl

        # You can set up any additional initialization here
    ...
    def _request(self, route, method, parameters=None):
        ...
        if "json" in r.headers["content-type"]:
            try:
                ...
                raise ValueError("Couldn't parse the JSON response received from the server: {}".format(r.content))
            ...
            if data.get("type"):
                ...
                if r.status_code == 400 and data["type"] == "error" and data["description"] == "Invalid Token":
                    ...
                    raise ValueError("Couldn't parse the JSON response received from the server: {}".format(data["description"]))
                ...
                if r.status_code == 400 and data["type"] == "error" and data["description"] == "Bad Request":
                    ...
                    raise ValueError("Couldn't parse the JSON response received from the server: {}".format(str(message)))

            return data
        else:
            raise ValueError("Unknown Content-Type ({content_type}) with response: ({content})".format(
