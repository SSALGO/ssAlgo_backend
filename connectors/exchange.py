...
class Exchange:
    def __init__(self, api,db,cred,reapi,sessionusertoken):
        ...
        if self.api is not None:
            ...
            trading_event(
                "legacy_market_websocket_start_skipped",
                force=True,
                reason="central_market_feed_manager_owns_websockets",
            )
        else:
            trading_event(
                "strategy_engine_stage",
    ...
    def _ensure_zerodha_market_data(self, user, api_key, access_token):
        ...
        trading_event(
            "broker_market_data_start_skipped",
            force=True,
            user=user,
            broker="zerodha",
            reason="central_market_feed_manager_owns_websockets",
        )
        return {"success": False, "status": "central_feed_only"}
        try:
    ...
    def _start_all_threads(self):
        ...
        trading_event(
            "strategy_engine_stage",
            force=True,
            stage="legacy_market_websocket_skipped",
            reason="central_market_feed_manager_owns_websockets",
        )
    ...
    def _dataequityscript(self):
        ...
        trading_event(
            "legacy_websocket_subscription_skipped",
            reason="central_market_feed_manager_owns_websockets",
            source="_dataequityscript",
        )
        while not self._shutdown_event.is_set():
            try:
                #print(self.prices)
    ...
    def OBUY(self, trade, OTYPE, Signal):
        try:
            ...
            if self.websocketretry > 10 and self.api is not None:
                trading_event(
                    "legacy_websocket_repair_skipped",
                    reason="central_market_feed_manager_owns_websockets",
                    symbol=option,
                )
                self.add_symbol_to_websocket(option)
    ...
    def _subscribe_aliceblue_depth_for_symbol(self, symbol, token=None, row=None):
        trading_event(
            "broker_market_data_subscription_skipped",
            broker="aliceblue",
            symbol=symbol,
            token=token,
            reason="central_market_feed_manager_owns_websockets",
    ...
    def _get_level1_depth_price(self, transaction_type, symbol, exch=None, optiontoken=None, require_fresh=True):
        ...
        try:
            ...
            if row:
                bid = self._first_positive_float(row.get('bid'))
                ask = self._first_positive_float(row.get('ask'))
                ltp = self._first_positive_float(row.get('ltp'))
                if side in ('BUY', 'B') and ask:
                    return self._round_price_to_tick(ask), row
                if side in ('SELL', 'S') and bid:
                    return self._round_price_to_tick(bid), row
                if ltp:
                    return self._round_price_to_tick(ltp), row
        except Exception as e:
            print(f"shared market depth lookup failed for {symbol}: {e}")
    ...
    def add_to_swebsocket(self, token):
        if type(token) == str:
            ...
            print('')
        trading_event(
            "legacy_snapshot_subscription_skipped",
            reason="central_market_feed_manager_owns_websockets",
            token=token,
        )
    ...
    def open_callback(self):
        ...
        trading_event(
            "legacy_websocket_open_subscription_skipped",
            reason="central_market_feed_manager_owns_websockets",
        )
            ...
            if row:
                ...
                if price is None:
                    ...
                    if bid and ask:
                        ...
                        price = ask or bid
                if price is not None and not self._is_suspicious_option_price(symbol, price):
                    self.prices[symbol] = float(price)
                    return float(price)
        except Exception as e:
            print(f"shared market price lookup failed for {symbol}: {e}")
    ...
    def contracts(self):
        ...
        if check:
            ...
            for zip_file in masters:
                ...
                target_path = contract_file_path(target_file)
                if target_path.exists() and not _env_bool(
                ...
                zip_path = CONTRACTS_DIR / zip_file
        ...
        Nse = pd.read_csv(contract_file_path('NSE_symbols.txt'))
        Nse = Nse.iloc[:, :-1]
        Bse = pd.read_csv(contract_file_path('BSE_symbols.txt'))
        Bse = Bse.iloc[:, :-1]
        Bfo = pd.read_csv(contract_file_path('BFO_symbols.txt'))
        Bfo = Bfo.iloc[:, :-1]
        Nfo = pd.read_csv(contract_file_path('NFO_symbols.txt'))
        Nfo = Nfo.iloc[:, :-1]
        Mcx = pd.read_csv(contract_file_path('MCX_symbols.txt'))
        Mcx = Mcx.iloc[:, :-1]
        Cds = pd.read_csv(contract_file_path('CDS_symbols.txt'))
        Cds = Cds.iloc[:, :-1]
        n = Nfo
