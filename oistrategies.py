...
class OIStrategy:
    def __init__(self, client, contract: Contract,
    ...
    def _open_position(self, signal_result: int):
        ...
        trade_size = self.balance_pct#self._client.get_trade_size(self.contract, self.candles[-1].close, self.balance_pct)
        ...
        self._add_log(f"{position_side.capitalize()} signal on {self.contract} {self.tf}")

        order_status = True#self._client.place_order(self.contract, "MARKET", trade_size, order_side)
        ...
        new_trade = Trade({"time": int(time.time() * 1000), "entry_price": self.candles[-1].close,
                            "contract": self.contract, "strategy": self.strat_name, "side": position_side,
                            "status": "open", "pnl": 0, "quantity": trade_size, "entry_id": 'none'})
        print(new_trade)
    ...
    def _check_tp_sl(self, trade: Trade):
        ...
        if tp_triggered or sl_triggered:
            ...
            self._add_log(f"{'Stop loss' if sl_triggered else 'Take profit'} for {self.contract} {self.tf} "
                          f"| Current Price = {price} (Entry price was {trade.entry_price})")

            order_side = "SELL" if trade.side == "long" else "BUY"

            order_status = True #self._client.place_order(self.contract, "MARKET", trade.quantity, order_side)

            if order_status is not None:
                self._add_log(f"Exit order on {self.contract} {self.tf} placed successfully")
                trade.status = "closed"
                self.ongoing_position = False


    def parse_trades(self,price,size):
        

        last_candle = self.candles[-1]
        pretimestamp=datetime.datetime.timestamp(last_candle.timestamp )
        # Same Candle
        timestamp=int(time.time()) 
        print(last_candle)

        if timestamp < pretimestamp + self.tf_equiv:
        ...
        elif timestamp >= + 2 * self.tf_equiv:

            missing_candles = int((timestamp - pretimestamp) / self.tf_equiv) - 1

            logger.info(" missing %s candles for %s %s (%s %s)",  missing_candles, self.contract,
                        self.tf, timestamp, last_candle.timestamp)

            pre=pretimestamp#(datetime.timestamp(datetime.fromtimestamp(last_candle.timestamp,tz=pytz.UTC))-((5*60*60)+(30*60)))
            timestamp
            start=int((timestamp-pre)/self.tf_equiv)
            new_candle = self._client.mt.get_historical_data(self.contract,self.tf,1,start)

            self.candles.extend(new_candle)
            ...
            new_candle = self._client.mt.get_historical_data(self.contract,self.tf,1,1)

            self.candles.extend(new_candle)

            logger.info(" New candle for %s %s", self.contract, self.tf)
...
class TechnicalStrategy(Strategy):
    def __init__(self, client, contract: Contract,  timeframe: str, balance_pct: float, take_profit: float,
